...
fn part_b_clever(file_path: &str) -> i32 {
    // Attempt to be clever with solving the problem: I tried to detect if a blockage would lead 
    // to a loop by tracking visited states and seeing if a blockage would lead to revisiting a 
    // state that's already been visited.  However, this didn't work because looping can happen by 
    // forcing the guard to follow an entirely new path
    let (mut map, mut loc) = parse_input(file_path);
    let mut visited = HashMap::new();
    for dir in ['^', '>', 'v', '<'] {
        visited.insert(dir, vec![vec![false; map[0].len()]; map.len()]);
    ...
    loop {
        ...
        if map[next_loc.0 as usize][next_loc.1 as usize] == '#' {
            ...
            map[next_loc.0 as usize][next_loc.1 as usize] = arrow;
            visited.get_mut(&arrow).unwrap()[loc.0 as usize][loc.1 as usize] = true;
            let (adj_loc, rot_arrow) = match arrow {
                '^' => ((loc.0, loc.1 + 1), '>'),
                '>' => ((loc.0 + 1, loc.1), 'v'),
                'v' => ((loc.0, loc.1 - 1), '<'),
                '<' => ((loc.0 - 1, loc.1), '^'),
            ...
            if !(adj_loc.0 < 0 || adj_loc.1 < 0 || adj_loc.0 > map.len() as i32 - 1 || adj_loc.1 > map[0].len() as i32 -1) {
                if visited.get(&rot_arrow).unwrap()[adj_loc.0 as usize][adj_loc.1 as usize] {
                    // println!("Found a loop by obstructing {loc:?}");
                    result += 1;
                }
            }
            loc = next_loc;
    ...
    for k in ['^', '>', 'v', '<'] {
        println!("Key is {k}");
        for row in 0..map.len() {
            let curr_row = &visited.get(&k).unwrap()[row];
            println!("{curr_row:?}");
        }
    }
